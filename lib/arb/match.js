...
// ── Precise election-WINNER matcher ──────────────────────────────────────
// The one clean cross-platform vein: Polymarket "Will <name> win <race>?"
// settles identically to Kalshi "<race> winner? — <name>". We pair them ONLY
// when the candidate AND the race (office + year) both match exactly. This
// rejects the look-alikes the token matcher falls for (president↔AG by shared
// name; Bitcoin↔CPI by shared "above"). Same person, same office, same cycle —
// or it's not a pair.
const OFFICES = [
  "presidential", "president", "senate", "house", "governor", "democratic",
  "republican", "nomination", "nominee", "attorney general", "prime minister",
  "mayor", "speaker", "chancellor", "parliament",
];
const POLY_WIN = /^will\s+(.+?)\s+win\s+(?:the\s+)?(.+?)\??$/i;

function parseName(s) {
  let n = String(s || "").toLowerCase()
    .replace(/\b[a-z]\.(\s|$)/g, " ")     // drop middle initials "J."
    .replace(/[^a-z\s]/g, " ")
    .replace(/\s+/g, " ").trim();
  const suf = /\b(jr|sr|ii|iii|iv)\b/.exec(n);
  const suffix = suf ? suf[1] : "";
  n = n.replace(/\b(jr|sr|ii|iii|iv)\b/g, "").replace(/\s+/g, " ").trim();
  const parts = n.split(" ").filter(Boolean);
  return { first: parts[0] || "", last: parts[parts.length - 1] || "", suffix };
}
function nameEq(a, b) {
  const x = parseName(a), y = parseName(b);
  if (!x.last || !y.last || x.last !== y.last) return false; // last name must match
  if (x.suffix !== y.suffix) return false;                   // Trump !== Trump Jr
  if (x.first && y.first && x.first[0] !== y.first[0]) return false; // first initial
  return true;
}
function raceSig(s) {
  const t = String(s || "").toLowerCase();
  return {
    year: (t.match(/\b20[2-3][0-9]\b/) || [""])[0],
    office: new Set(OFFICES.filter((o) => t.includes(o))),
  };
}
function raceMatch(a, b) {
  if (a.year && b.year && a.year !== b.year) return false;    // different cycle
  if (!a.office.size || !b.office.size) return false;          // need an office on both
  // "Win the nomination" ≠ "win the presidency" — distinct contracts.
  const nom = (x) => x.office.has("nomination") || x.office.has("nominee");
  if (nom(a) !== nom(b)) return false;
  // A party-specific race must agree on party.
  if (nom(a) || nom(b)) {
    for (const party of ["democratic", "republican"]) {
      if (a.office.has(party) !== b.office.has(party)) return false;
    }
  }
  for (const o of a.office) if (b.office.has(o)) return true;  // shared office
  return false;
}

/** @returns Array<{ poly, kalshi, match_score, window, same_threshold }> */
export function matchWinners(polyMarkets, kalshiMarkets) {
  const kCands = [];
  for (const k of kalshiMarkets) {
    const idx = (k.title || "").lastIndexOf(" — ");
    if (idx < 0) continue;
    const race = k.title.slice(0, idx);
    const cand = k.title.slice(idx + 3).trim();
    if (cand.length < 3 || /\b(yes|no|above|below|or more|or less)\b/i.test(cand)) continue;
    kCands.push({ market: k, cand, sig: raceSig(race) });
  }
  const pairs = [];
  const used = new Set();
  for (const p of polyMarkets) {
    const m = (p.title || "").match(POLY_WIN);
    if (!m) continue;
    const cand = m[1].trim();
    if (cand.split(/\s+/).length > 4) continue;   // candidate names are short
    const sig = raceSig(m[2]);
    if (!sig.office.size) continue;
    for (const kc of kCands) {
      if (used.has(kc.market.id)) continue;
      if (!nameEq(cand, kc.cand) || !raceMatch(sig, kc.sig)) continue;
      pairs.push({ poly: p, kalshi: kc.market, match_score: 0.96, window: "same", same_threshold: true });
      used.add(kc.market.id);
      break;
    }
  }
  return pairs;
