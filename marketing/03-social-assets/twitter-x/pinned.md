# X / Twitter — Pinned Thread (6 tweets)

Pin tweet #1. Replies form the thread. Each tweet is hand-typed (don't auto-thread from a tool — the cadence reads as bot).
...
## Pinning notes
- Pin tweet 1. The reply-thread holds the rest.
- Add the site link as a CARD in tweet 1 (paste URL, let X unfurl, then delete the raw URL text after the unfurl renders).
- Quote-pin tweet 4 (divergence demo) once a week with that day's actual reading. Keep the thread alive.
