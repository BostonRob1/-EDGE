# Medium — Publication

## Publication name
$EDGE / Notes

## Tagline (160 char)
Field notes from the edge of prediction markets. Money. Mouth. Edge. Long-form essays, dossiers, and devlogs from the team building the $EDGE terminal.

## Description (short)
Long-form prediction-market intelligence. Polymarket dossiers, Kalshi breakdowns, whale autopsies, divergence playbooks, founder devlogs.

## URL slug
medium.com/edge-notes (or /at-edge if -notes is taken)

## Logo (header)
`pfp.svg` — 1500×750 wide horizontal logo (Medium uses this as the masthead).

## Cover (homepage hero)
`banner.svg` — 1500×750.

## Categories
- Cryptocurrency
- Finance
- Politics
- Data Science

## Writers
- Rob Noejr — `@robertnoejr` (intro: "Building $EDGE — a terminal for prediction markets.")
- Tye — `@tye` (intro: "Data + infra at $EDGE. The market doesn't lie; the data sometimes does.")

## Contributor application gate
Closed for v1. Founders write everything for the first 90 days. Open submissions after the publication has 10+ posts and a reputation surface.

## SEO meta
- Title: $EDGE / Notes — Prediction-Market Intelligence
- Description: Long-form essays and dossiers on Polymarket, Kalshi, whales, and the gap between money and mouth.

## Bio variants for cross-post links

### 80 char
Long-form prediction-market intelligence. Read the field notes.

### 160 char
$EDGE / Notes — long-form essays on Polymarket + Kalshi, whale autopsies, divergence playbooks. Field notes from the edge of prediction markets.
