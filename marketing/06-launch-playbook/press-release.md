EMBARGOED UNTIL: [LAUNCH DATE] 12:00 UTC

---

# $EDGE Launches — The First Bloomberg-Terminal-Grade Aggregator for Prediction Markets
...
The moat is built on three signal layers fused into a single screen: **Money** (live order-book and last-price data from Polymarket and Kalshi side-by-side, plus a live **cross-platform arbitrage radar** that surfaces fee-adjusted price discrepancies between the two venues), **Mouth** (sentiment aggregated from X, Reddit, and forums), and **Edge** (the divergence score — a proprietary metric that flags when the public narrative diverges from where the actual capital is sitting). No incumbent owns this layer. The closest competitors are single-platform stat sites; none ingest cross-platform price, cross-platform whale flow, cross-platform arbitrage, and public sentiment in one view. $EDGE also ships with a public, append-only track record of every divergence call the system makes — receipts by design.

The $EDGE token launches today via **fairlaunch on pump.fun**, with a fixed supply of 1 billion tokens. There is no presale, no private round, and **no team allocation** — 100% of supply enters the open market on the pump.fun bonding curve and migrates to Raydium at approximately $69K market cap, with $12K of liquidity permanently burned at migration. Token utility unlocks tiered terminal features by holdings: **100K $EDGE** for the Trader tier (real-time whale alerts, custom firehose thresholds, deep Rug Radar scans), **1M $EDGE** for the Analyst tier (divergence radar, public track-record ledger, API access), and **5M $EDGE** for the Desk tier (custom backtesting, unlimited priority API, affiliate revenue share, governance vote). The free public tier (dashboard, hot markets, buzz heat, delayed whale firehose) requires no holdings.
